...
def test_xno_status_after_click_tabs(login_direct_and_get_page):
    ...
    def handle_response(response):
        if response.request.resource_type in ["xhr", "fetch"]:
            ...
            print(f"<< {status} {response.url} (Tab: {tab_name})")
    ...
    print("\n=== KẾT QUẢ THỐNG KÊ RESPONSE SAU KHI CLICK TABS ===")
    for tab, counts in status_counts_per_tab.items():
        print(f"\nTab: {tab}")
        for group in sorted(counts):
            prefix = "" if group == 200 else ("" if group == 400 else "")
